...
namespace HooksDefinitions
{
	...
	//RVA
	uintptr_t offsetNetworkPlayerStart = 0x1230E60; // -> <Player>.Start()
	uintptr_t offsetNetworkPlayerDestroy = 0x122AA30; // -> <Player>.OnDestroy()
	uintptr_t offsetWorldToScreenPoint = 0x1D02A40; // -> <Camera>.WorldToScreenPoint(Vector3)
	uintptr_t offsetScreenToWorldPoint = 0x1D02650; // -> <Camera>.ScreenToWorldPoint(Vector3)
	uintptr_t offsetLocalPlayerStart = 0x10F5260; // -> <LocalPlayer>.Start()
